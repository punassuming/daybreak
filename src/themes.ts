import { Palette, generateLightFromDark, generateDarkFromLight, hexToRgb, rgbToHex, invertLightness } from './colors';

export type ThemePaletteSet = { dark?: Palette; light?: Palette };

export const NORD: ThemePaletteSet = {
  dark: {
...
export const GRUVBOX: ThemePaletteSet = {
  dark: {
...
export const DRACULA: ThemePaletteSet = {
  dark: {
...
export const SOLARIZED: ThemePaletteSet = {
  dark: {
...
export const CATPPUCCIN: ThemePaletteSet = {
  dark: {
...
export const TOKYO_NIGHT: ThemePaletteSet = {
  dark: {
...
export const MONOKAI: ThemePaletteSet = {
  dark: {
...
export const ONE_DARK: ThemePaletteSet = {
  dark: {
...
export const THEME_LIBRARY: Record<string, ThemePaletteSet> = {
  'Nord': NORD,
  'Gruvbox': GRUVBOX,
  'Dracula': DRACULA,
  'Solarized': SOLARIZED,
...
export function enrichPaletteDepth(palette: Palette, mode: string): void {
  if (!palette.colors) palette.colors = {};

  const bg = palette.special.background;
  const fg = palette.special.foreground;

  function adjustBg(hexC: string, amount: number): string {
    const [r, g, b] = hexToRgb(hexC);
    // Simple lightness adjustment
    const factor = mode === 'dark' ? amount : -amount;
    return invertLightness(hexC, Math.max(0, Math.min(1, (r + g + b) / (3 * 255) + factor)));
  }

  if (!('18' in palette.colors)) {
    palette.colors['18'] = adjustBg(bg, 0.10);
  }
  if (!('19' in palette.colors)) {
    palette.colors['19'] = adjustBg(bg, 0.20);
  }
  if (!('16' in palette.colors) && '11' in palette.colors) {
    palette.colors['16'] = palette.colors['11'];
  }
  if (!('20' in palette.colors)) {
    palette.colors['20'] = palette.colors['8'] || fg;
  }
  if (!('21' in palette.colors)) {
    palette.colors['21'] = fg;
  }
}

export function getThemePalette(themeName: string): { light: Palette; dark: Palette } {
  const base = THEME_LIBRARY[themeName] || NORD;
  const result: { light?: Palette; dark?: Palette } = { ...base };
  ...
  for (const mode of ['light', 'dark'] as const) {
    if (result[mode]) {
      enrichPaletteDepth(result[mode]!, mode);
    }
  }

  return result as { light: Palette; dark: Palette };
}
